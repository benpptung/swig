...
/**
 * Shortcut to render the data passed by server to front-end as an inline <script> element with a non-javascript type
 *
 * @example
 * // val = { a: 'b' }
 * <script type="application/json" id="jsondata">{{ val|jsondata }}</script>
 *
 * // in client side
 * // var json = JSON.parse(document.getElementById('jsondata').innerHtml);
 * // => {"a":"b"}
 *
 *
 * @param  {*}    input
 * @return {string}           A valid JSON string.
 */
exports.jsondata = function (input) {
  return JSON.stringify(input).replace(/<\/script/g, '<\\/script').replace(/<!--/g, '\u003C!--');
};

exports.jsondata.safe = true;
